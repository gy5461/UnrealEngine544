	...
	// Draw boxes to show frames
	{
		...
		if (NumFrames > 0)
		{
			...
			for (int32 i = 0; i < NumFrames; ++i)
			{
				...
				FSlateDrawElement::MakeBox(OutDrawElements, LayerId++, FSlateInvalidationWidgetSortOrder(),AllottedGeometry.ToPaintGeometry(BoxSize, FSlateLayoutTransform(BoxLocation)), BoxBrush, ESlateDrawEffect::None, Tint);
			}
		}
		else
		{
			FSlateDrawElement::MakeBox(OutDrawElements, LayerId++,FSlateInvalidationWidgetSortOrder(), AllottedGeometry.ToPaintGeometry(), BoxBrush, ESlateDrawEffect::None, BoxTints[0]);
		}
	}
